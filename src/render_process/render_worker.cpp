...
void RenderWorker::executeStoppable(std::shared_future<void> futureObject) {
	...
	while (!stopThread && tilePool->getPoolSize() > 0)
	{
		Tile tile = tilePool->getNextTile();

		for (int j = tile.yMax - 1; !stopThread && j >= tile.yMin; j--) {

			for (int i = tile.xMin; i < tile.xMax; i++) {
				vec3 col(0, 0, 0);
				Point2i currentPixel(i, j);
				randomSampler.startPixel(currentPixel);
				if(futureObject.wait_for(std::chrono::seconds(0)) == std::future_status::ready){
					stopThread = true;
					break;
				}
				for (int s = 0; s < renderSamples; s++) {
