...
texture* SceneParser::parseTexture(json textureJSON) {
	...
	if (textureJSON["type"] == "constantTexture") {
		currentTexture = new constant_texture(vec3(textureJSON["color"][0], textureJSON["color"][1], textureJSON["color"][2]));
	}
...
material* SceneParser::parseMaterial(json materialJSON) {
	...
	{
		currentMaterial = new metal(vec3(materialJSON["color"][0], materialJSON["color"][1], materialJSON["color"][2]), materialJSON["fuzz"]);
	}
...
void SceneParser::buildMaterialList() {
	...
	for (json::iterator it = JSONObject["materialList"].begin(); it != JSONObject["materialList"].end(); ++it, ++count)
	{
...
hitable* SceneParser::parseObject(json objectJSON) {
	...
	else if (objectJSON["type"] == "sphere")
	{
		currentObject = new sphere(
			vec3(objectJSON["center"][0], objectJSON["center"][1], objectJSON["center"][2]),
			objectJSON["radius"],
			materialList[materialIndex]);
	}
	else if (objectJSON["type"] == "sphere")
	...
	else if (objectJSON["type"] == "box")
	{
		currentObject = new box(
			vec3(objectJSON["p0"][0], objectJSON["p0"][1], objectJSON["p0"][2]),
			vec3(objectJSON["p1"][0], objectJSON["p1"][1], objectJSON["p1"][2]),
			materialList[materialIndex]);
	}
	...
	vec3 objectLocation(objectJSON["location"][0], objectJSON["location"][1], objectJSON["location"][2]);
...
void SceneParser::parseScene() {
	...
	vec3 lookfrom(JSONObject["camera"]["lookfrom"][0], JSONObject["camera"]["lookfrom"][1], JSONObject["camera"]["lookfrom"][2]);
	vec3 lookat(JSONObject["camera"]["lookat"][0], JSONObject["camera"]["lookat"][1], JSONObject["camera"]["lookat"][2]);
	float focalDist = JSONObject["camera"]["focalDist"];
	Camera cam(lookfrom, lookat, vec3(0, 1, 0), verticalFieldOfView, float(imageWidth) / float(imageHeight), aperture, focalDist, 0.0, 1.0);
	scene.camera = cam;
